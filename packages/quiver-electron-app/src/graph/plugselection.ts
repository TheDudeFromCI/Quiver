...
export class PlugSelection implements MouseListener {
  ...
  render (): void {
    ...
    if (this.selectedPlug.isInput) this.plugPos.set(node.pos.x, node.pos.y + visualIndex * Theme.NODE_PLUG_HEIGHT)
    else this.plugPos.set(node.pos.x + node.width, node.pos.y + (visualIndex + node.type.inputs.length) * Theme.NODE_PLUG_HEIGHT)
  ...
  mouseUp (mouse: MouseInfo): void {
    ...
    const node = this.nodeHandler.getNodeAt(mouse.pos)
    const plugIndex = node?.getPlugAt(mouse.pos) ?? -1
  ...
  private canPlaceConnection (node: GraphNode, plugIndex: number): boolean {
    ...
    if (isTargetInput) return !this.nodeHandler.isNodeDecendantOf(this.selectedPlug.node, node)
    else return !this.nodeHandler.isNodeDecendantOf(node, this.selectedPlug.node)
  }
}
