...
export class ContextWindow {
  ...
  updatePosition (menuPos: Position): void {
    ...
    for (const child in this.children) this.children[child].updatePosition(menuPos)
  }
  ...
  render (): void {
    ...
    for (let i = 0; i < this.options.length; i++) ctx.fillText(this.options[i], this.pos.x + 3, this.pos.y + (i + 0.5) * Theme.CONTEXT_WINDOW_LINE_HEIGHT)
  ...
  isInBounds (pos: Position, recursive: boolean): boolean {
    if (pos.x >= this.pos.x && pos.x < this.pos.x + this.width &&
            pos.y >= this.pos.y && pos.y < this.pos.y + this.height) return true

    if (recursive) {
      for (const child in this.children) {
        if (this.children[child].isInBounds(pos, recursive)) return true
      }
    }
  ...
  getOptionAt (pos: Position): string | null {
    ...
    for (const option of this.options) {
      if (pos.y >= y && pos.y < y + Theme.CONTEXT_WINDOW_LINE_HEIGHT) return option
  ...
  updateSelection (pos: Position, recursive: boolean): boolean {
    ...
    if (this.selectedOption != null) this.renderedChild = this.children[this.selectedOption]
    if (recursive) updated ||= this.renderedChild?.updateSelection(pos, recursive) ?? false
  ...
  closeAll (): void {
    ...
    for (const child in this.children) this.children[child].closeAll()
  }
  ...
  selectOptionAt (mousePos: Position, recursive: boolean): void {
    ...
    if (recursive) this.renderedChild?.selectOptionAt(mousePos, recursive)
  }
}
...
export class ContextMenu implements MouseListener {
  ...
  addOption (option: string, action: ContextMenuAction): void {
    const onPress = (mp: Position): void => {
      this.close()
      action(mp)
    }
  ...
  mouseDown (mouse: MouseInfo): void {
    ...
    if (mouse.button === 0) this.selectOptionAt(mouse.screenPos)
  }

  mouseMoved (mouse: MouseInfo): void {
    if (this.isVisible) this.updateSelection(mouse.screenPos)
  }
