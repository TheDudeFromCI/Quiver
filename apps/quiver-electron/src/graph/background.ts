...
export class Background {
  ...
  render (): void {
    ...
    if (Theme.DRAW_GRID) { this.renderGrid() }
  }
  ...
  private renderLines (ctx: CanvasRenderingContext2D, step: number): void {
    const minX = Math.ceil(this.bounds.min.x / step) * step
    const maxX = Math.ceil(this.bounds.max.x / step) * step
    const minY = Math.ceil(this.bounds.min.y / step) * step
    const maxY = Math.ceil(this.bounds.max.y / step) * step
